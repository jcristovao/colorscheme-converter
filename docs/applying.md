...
Three commands, in increasing order of permanence: `live` recolours the
terminal you are sitting in, `activate` installs a scheme so it survives a
restart, and `active` tells you what each application ended up using.
...
protocol. That was the deciding factor: kitty's `@ set-colors` is excellent
but needs `allow_remote_control` turned on and only works for kitty, whereas
OSC works in every terminal here with nothing configured. Sequences go to
...
the copy holding your untouched original. `--no-backup` skips this.
...
command fails having changed nothing. `--no-validate` skips this.
...
**Terminals are read, and resolve to a file** — so the answer can be parsed
and shown as colors. Getting this right needs more than a grep: kitty and foot
...
In [`cscx browse`](browsing.md), schemes a terminal is currently using are
marked in the list with a green dot.
